...
ngl_full_list = "https://geodesy.unr.edu/NGLStationPages/llh.out"
ngl_24h_2w = "https://geodesy.unr.edu/NGLStationPages/DataHoldings.txt"
ngl_24h_24h = "https://geodesy.unr.edu/NGLStationPages/DataHoldingsRapid24hr.txt"
ngl_5min_24h = "https://geodesy.unr.edu/NGLStationPages/DataHoldingsRapid5min.txt"
ngl_5min_1h30min = "https://geodesy.unr.edu/NGLStationPages/DataHoldingsUltra5min.txt"
east_ref_name = "___e-ref(m)"
north_ref_name = "___n-ref(m)"
vertical_ref_name = "___v-ref(m)"
east_mean_name = "_e-mean(m)"
north_mean_name = "_n-mean(m)"
vertical_mean_name = "_v-mean(m)"
east_sigma_name = "sig_e(m)"
north_sigma_name = "sig_n(m)"
vertical_sigma_name = "sig_v(m)"


ngl_columns_rename_dict = {east_ref_name : 'east_ref',
               north_ref_name : 'north_ref',
               vertical_ref_name : 'vertical_ref',
               east_mean_name : 'east_mean',
               north_mean_name : 'north_mean',
               vertical_mean_name : 'vertical_mean',
               east_sigma_name : 'east_sigma',
               north_sigma_name : 'north_sigma',
               vertical_sigma_name : 'vertical_sigma'}
...
def ngl_retrieve_24h(rootpath, station_name,force_download=False):
    ...
    base_url = "https://geodesy.unr.edu/gps_timeseries/" + data_type
    ...
    data['date'] = [str_to_datetime(str(s), 50) for s in data['YYMMMDD']]
...
def ngl_rapid_5min(station,year):
    url = "https://geodesy.unr.edu/gps_timeseries/kenv/%s/%s.%4i.kenv.zip"
    try:
    ...
    for nl in zip_file_object.namelist():
        ...
        with gzip.open(file, 'rb') as f:
            ...
            content_df = pd.read_csv(StringIO(content), sep=r"\s+")
            doy = content_df['doy'].values[0]
            dataframes[doy] = content_df            
    return dataframes


def ngl_rapid_5min_dataframes_merge_postprocess(dataframes_dict, year):
    dfs = []
    ref_day = datetime.datetime(year,1,1)
    for doy, df in dataframes_dict.items():
        day = ref_day + datetime.timedelta(days=float(doy-1))
        new_df = df.copy()
        new_df['date'] = day
        new_df['datetime'] = [ day + datetime.timedelta(seconds = s)  for s in new_df['s-day']]
        dfs.append(new_df)
    return pd.concat(dfs).rename(ngl_columns_rename_dict, axis=1).sort_values('datetime', axis=0).drop(['sec-J2000','__MJD','year','mm','dd','doy','s-day'], axis=1)
